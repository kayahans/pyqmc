...
class MultiplyWF:
    ...
    def gradient(self, e, epos,configs=None):
...
class MultiplyBosonWF(MultiplyWF):
    ...
    def value(self):
        results = [wf.value() for wf in self.wf_factors]
        results = np.array([*results])
        return np.prod(results[:, 0, :], axis=0), np.prod(results[:, 1, :], axis=0)
