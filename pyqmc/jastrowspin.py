...
#(Kayahan) Below is mostly from the JastrowSpin class above, but converted to Boson in non-log wavefunctions 
class BosonJastrowSpin:
    ...
    def recompute(self, configs):
        ...
        u = gpu.cp.sum(self._bvalues * self.parameters["bcoeff"], axis=(2, 1))
        u += gpu.cp.einsum("ijkl,jkl->i", self._avalues, self.parameters["acoeff"])
        val = np.exp(u) #(kayahan) Updated from Jastrowspin
