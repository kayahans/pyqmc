#!/usr/bin/env python3.12
"""Boson ABDMC driver.

Local/Mac: default ``parallel_backend: process`` in config.yaml uses
ProcessPoolExecutor with OMP=1. Cluster: set ``parallel_backend: mpi`` and
launch via job.sh (mpirun -m mpi4py.futures).
"""
import os
import sys
import time
import copy
import concurrent.futures
import multiprocessing as mp
import yaml
import h5py

import pyqmc.gpu as gpu
import pyqmc.pyscftools as pyscftools
from pyqmc import bosonwftools
from pyqmc import bosonrecipes

# Wavefunction generation
continue_qmc = True
reuse = True
# To profile the code
# profile_boson_dmc: true
# profile_abcdmc_print_every: 10

with open("config.yaml", "r") as f:
    config = yaml.safe_load(f)

basis_name = config["basis_name"]
ecp = config.get("ecp", None)
checkfile_dir = config.get("checkfile_dir", ".")
symm_tag = config["symm_tag"]
det_emax = config["det_emax"]
dtwarmup = config.get("dtwarmup", 0.5)
dmc_eq_dt = config.get("dmc_eq_dt", 0.02)
dmc_stat_dt = config.get("dmc_stat_dt", 0.01)
dmc_stat_nstep = config.get("dmc_stat_nstep", 500)
vmc_nstep = config.get("vmc_nstep", 200)
vmc_nconfig = config.get("vmc_nconfig", 512)
dmc_eq_nblocks = config.get("dmc_eq_nblocks", 200)
dmc_stat_nconfig = config.get("dmc_stat_nconfig", 512)
dmc_stat_nsteps_per_block = config.get("dmc_stat_nsteps_per_block", 5)
xc = config.get("xc", "PBE, PBE")
ion_cusp = config["ion_cusp"]
na = config["na"]
nb = config["nb"]
rcut = config["rcut"]
parallel_backend = config.get("parallel_backend", "process")
npartitions_cfg = config.get("npartitions", None)
evaluate_mf_with = config.get("evaluate_mf_with", "numba")
# exclude_core = config.get('exclude_core', False)

from pyqmc.observables.bosonaccumulators import configure_boson_dmc_profiling

configure_boson_dmc_profiling(
    enabled=config.get("profile_boson_dmc"),
    print_every=config.get("profile_abcdmc_print_every"),
)


def print_config():
    print("Printing configuration...")
    print("--------------------------------")
    print(f"dtwarmup: {dtwarmup}")
    print(f"dmc_eq_dt: {dmc_eq_dt}")
    print(f"dmc_stat_dt: {dmc_stat_dt}")
    print(f"vmc_nstep: {vmc_nstep}")
    print(f"vmc_nconfig: {vmc_nconfig}")
    print(f"dmc_eq_nblocks: {dmc_eq_nblocks}")
    print(f"dmc_stat_nconfig: {dmc_stat_nconfig}")
    print(f"dmc_stat_nstep: {dmc_stat_nstep}")
    print(f"dmc_stat_nsteps_per_block: {dmc_stat_nsteps_per_block}")
    print(f"xc: {xc}")
    print(f"ion_cusp: {ion_cusp}")
    print(f"na: {na}")
    print(f"nb: {nb}")
    print(f"rcut: {rcut}")
    print(f"det_emax: {det_emax}")
    print(f"symm_tag: {symm_tag}")
    print(f"basis_name: {basis_name}")
    print(f"checkfile_dir: {checkfile_dir}")
    print(f"reuse: {reuse}")
    print(f"continue_qmc: {continue_qmc}")
    print(f"parallel_backend: {parallel_backend}")
    print(f"evaluate_mf_with: {evaluate_mf_with}")
    print("Configuration printed")
    print("--------------------------------")


def write_mock_opt_hdf5(
    dft_checkfile,
    ci_checkfile,
    output_hdf5,
    jastrow_kws,
    det_emax,
    use_symm=True,
):
    """Build Boson×Jastrow WF (same recipe as ABDMC) and save ``wf/*`` parameters for read_wf.

    Layout matches bosonlinemin / wftools.read_wf: group ``wf`` with one dataset per
    parameter key.
    """
    target_root = 0
    mol, mf, mc = pyscftools.recover_pyscf(dft_checkfile, ci_checkfile=ci_checkfile)
    if not hasattr(mc.ci, "shape") or len(mc.ci.shape) == 3:
        mc.fci = mc.ci
        mc.ci = mc.ci[target_root]

    wf, _ = bosonwftools.generate_boson_wf(
        mol,
        mf,
        mc=mc,
        jastrow_kws=jastrow_kws,
        slater_kws={},
        det_emax=det_emax,
        use_symm=use_symm,
    )
    with h5py.File(output_hdf5, "w") as hdf:
        hdf.create_group("wf")
        for k in wf.parameters.keys():
            hdf.create_dataset("wf/" + k, data=gpu.asnumpy(wf.parameters[k]))


def setup_parallel_environment(backend):
    """Pin BLAS/OpenMP threads.

    Process pools: force 1 thread per process to avoid oversubscription.
    MPI: honor OMP_NUM_THREADS from the environment (job.sh sets 1).
    Serial: leave a modest default if unset.
    """
    if backend == "process":
        nthreads = 1
    elif backend == "mpi":
        nthreads = int(os.environ.get("OMP_NUM_THREADS", "1"))
    else:
        nthreads = int(os.environ.get("OMP_NUM_THREADS", "1"))

    for key in (
        "OMP_NUM_THREADS",
        "MKL_NUM_THREADS",
        "OPENBLAS_NUM_THREADS",
        "NUMEXPR_NUM_THREADS",
    ):
        os.environ[key] = str(nthreads)

    print(f"Running with {nthreads} threads per process (backend={backend})")
    return nthreads


def resolve_npartitions(backend, npartitions_cfg):
    """Return (npartitions, optional MPI rank)."""
    if backend == "serial":
        return 0, None
    if backend == "mpi":
        from mpi4py import MPI

        comm = MPI.COMM_WORLD
        return max(1, comm.Get_size() - 1), comm.Get_rank()
    # process
    if npartitions_cfg is not None:
        return int(npartitions_cfg), None
    return max(1, mp.cpu_count() - 1), None


class _NullPool:
    """Context manager that yields None (serial path)."""

    def __enter__(self):
        return None

    def __exit__(self, *args):
        return False


def make_client(backend, npartitions):
    """Context manager yielding (client, npartitions) for recipe calls."""
    if backend == "serial":
        return _NullPool()
    if backend == "mpi":
        from mpi4py.futures import MPIPoolExecutor

        return MPIPoolExecutor(max_workers=npartitions)
    if backend == "process":
        return concurrent.futures.ProcessPoolExecutor(max_workers=npartitions)
    raise ValueError(
        f"parallel_backend={backend!r} not recognized; use process|mpi|serial"
    )


def run_with_client(backend, npartitions, fn, *args, **kwargs):
    """Call recipe fn; attach client/npartitions when not serial."""
    if backend == "serial":
        print("Using Serial code")
        return fn(*args, **kwargs)
    print(f"Using Parallel code ({backend}, npartitions={npartitions})")
    with make_client(backend, npartitions) as client:
        return fn(
            *args,
            client=client,
            npartitions=npartitions,
            **kwargs,
        )


if __name__ == "__main__":
    if parallel_backend not in ("process", "mpi", "serial"):
        raise ValueError(
            f"parallel_backend={parallel_backend!r} not recognized; "
            "use process|mpi|serial"
        )

    # Prefer spawn on macOS before any ProcessPool (Numba/PySCF safety).
    if parallel_backend == "process" and sys.platform == "darwin":
        try:
            mp.set_start_method("spawn", force=True)
        except RuntimeError:
            pass

    start_time = time.time()
    print(f"=== Starting QMC calculation at {time.strftime('%Y-%m-%d %H:%M:%S')} ===")
    print_config()
    setup_parallel_environment(parallel_backend)
    npartitions, mpi_rank = resolve_npartitions(parallel_backend, npartitions_cfg)
    print(f"npartitions: {npartitions}")

    atom_name = config["atom_name"]
    ncas = config["ncas"]
    nelecas = tuple(config["nelecas"])

    file_template = (
        "{}_atom_basis_{}_diffuse_S0P0D0_v1{}.hdf5 "  # scf
        "{}_ci_atom_basis_{}_diffuse_S0P0D0{}.hdf5 "  # ci
        "{}_opt_cas_{}_nelecas_{}_{}.hdf5 "  # opt
        "{}_dmc_eq_cas_{}_nelecas_{}_{}.hdf5 "  # dmc eq
        "{}_dmc_cas_{}_nelecas_{}_{}.hdf5"  # dmc
    )

    files_str = file_template.format(
        atom_name,
        basis_name,
        symm_tag,
        atom_name,
        basis_name,
        symm_tag,
        atom_name,
        ncas,
        nelecas[0],
        nelecas[1],
        atom_name,
        ncas,
        nelecas[0],
        nelecas[1],
        atom_name,
        ncas,
        nelecas[0],
        nelecas[1],
    )

    scf_checkfile, ci_checkfile, opt_checkfile, abvmc_eq_checkfile, abvmc_checkfile = (
        files_str.split()
    )
    scf_checkfile = os.path.join(checkfile_dir, scf_checkfile)
    ci_checkfile = os.path.join(checkfile_dir, ci_checkfile)
    for path in (scf_checkfile, ci_checkfile):
        if not os.path.isfile(path):
            raise FileNotFoundError(f"Missing checkpoint: {path}")
    print(f"SCF checkpoint: {scf_checkfile}")
    print(f"CI checkpoint:  {ci_checkfile}")

    vmc_equilibrium = {
        "nconfig": vmc_nconfig,
        "nsteps_per_block": 1,
        "nblocks": vmc_nstep,
        "tstep": dtwarmup,
        "hdf_file": "warmup_vmc.hdf5",
        "accumulators": ["energy", "radial_density"],
    }
    dmc_equilibrium = {
        "nconfig": vmc_equilibrium["nconfig"],
        "nblocks": dmc_eq_nblocks,
        "tstep": dmc_eq_dt,
        "hdf_file": "warmup_dmc.hdf5",
        "accumulators": ["energy"],
    }
    dmc_statistics = {
        "nconfig": dmc_stat_nconfig,
        "nblocks": dmc_stat_nstep,
        "tstep": dmc_stat_dt,
    }

    jastrow_kws = {
        "ion_cusp": False,
        "na": na,
        "nb": nb,
        "rcut": rcut,
    }
    if not reuse:
        print("RUNNING ABVMC OPTIMIZATION with single determinant")
        num_int = 20
        aboptimize_params = {
            "ci_checkfile": ci_checkfile,
            "max_iterations": num_int,
            "jastrow_kws": jastrow_kws,
            "verbose": True,
            "nconfig": dmc_stat_nconfig,
            "det_emax": det_emax,
            "use_symm": True,
            "use_dft_density": True,
            "opt_method": "linemin",
            "evaluate_mf_with": evaluate_mf_with,
            "vmcoptions": {
                "nsteps_per_block": 10,
                "nblocks": 10,
                "tstep": dtwarmup,
            },
            "warmup_options": vmc_equilibrium,
        }

        opt_start_time = time.time()
        print(
            f"=== Starting Jastrow optimization at {time.strftime('%H:%M:%S')} ==="
        )
        run_with_client(
            parallel_backend,
            npartitions,
            bosonrecipes.ABOPTIMIZE,
            scf_checkfile,
            opt_checkfile,
            **aboptimize_params,
        )
        opt_end_time = time.time()
        opt_duration = opt_end_time - opt_start_time
        print(
            f"=== Jastrow optimization completed in {opt_duration:.2f} seconds "
            f"({opt_duration/60:.2f} minutes) ==="
        )
    else:
        print("Reusing jastrows")
        if not os.path.isfile(opt_checkfile):
            print("Writing mock opt checkpoint (fixed Jastrow, no line minimization)")
            opt_start_time = time.time()
            write_mock_opt_hdf5(
                scf_checkfile,
                ci_checkfile,
                opt_checkfile,
                jastrow_kws,
                det_emax,
                use_symm=True,
            )
            opt_end_time = time.time()
            print(
                f"=== Mock opt file written to {opt_checkfile} in "
                f"{opt_end_time - opt_start_time:.2f} s ==="
            )

    if not continue_qmc:
        for fname in [abvmc_checkfile]:
            if os.path.isfile(fname):
                os.remove(fname)

    vmc_eq_noconfig = vmc_equilibrium.copy()
    vmc_eq_noconfig.pop("nconfig")
    abdmc_params_eq = {
        "ci_checkfile": ci_checkfile,
        "verbose": True,
        "jastrow_kws": jastrow_kws,
        "tstep": dmc_equilibrium["tstep"],
        "nconfig": dmc_equilibrium["nconfig"],
        "nblocks": dmc_equilibrium["nblocks"],
        "vmc_options": vmc_eq_noconfig,
        "nsteps_per_block": 1,
        "load_parameters": opt_checkfile,
        "initial_guess_r": 10,
        "xc": xc,
        "use_dft_density": True,
        "use_symm": True,
        "det_emax": det_emax,
        "evaluate_mf_with": evaluate_mf_with,
    }
    n_eq = dmc_equilibrium["nconfig"]
    n_stat = dmc_statistics["nconfig"]
    if n_stat % n_eq != 0:
        raise ValueError(
            f"dmc_stat_nconfig ({n_stat}) must be a multiple of eq nconfig ({n_eq}) "
            "for population_snapshots assembly"
        )
    abdmc_params_eq["population_snapshots"] = n_stat // n_eq

    abdmc_params = copy.deepcopy(abdmc_params_eq)
    abdmc_params.pop("population_snapshots", None)
    abdmc_params.pop("vmc_options", None)
    abdmc_params["accumulators"] = ["abc_dmc_excitations"]
    abdmc_params["start_from"] = abvmc_eq_checkfile
    abdmc_params["nconfig"] = dmc_statistics["nconfig"]
    abdmc_params["nblocks"] = dmc_statistics["nblocks"]
    abdmc_params["tstep"] = dmc_statistics["tstep"]
    abdmc_params["nsteps_per_block"] = dmc_stat_nsteps_per_block

    abdmc_start_time = time.time()
    print(
        f"=== Starting ABDMC EQUILIBRIUM calculation at {time.strftime('%H:%M:%S')} ==="
    )
    run_with_client(
        parallel_backend,
        npartitions,
        bosonrecipes.ABDMC,
        scf_checkfile,
        abvmc_eq_checkfile,
        **abdmc_params_eq,
    )
    abdmc_end_time = time.time()
    abdmc_duration = abdmc_end_time - abdmc_start_time
    print(
        f"=== ABDMC EQUILIBRIUM calculation completed in {abdmc_duration:.2f} seconds "
        f"({abdmc_duration/60:.2f} minutes) ==="
    )

    abdmc_start_time = time.time()
    print(
        f"=== Starting ABDMC STATISTICS calculation at {time.strftime('%H:%M:%S')} ==="
    )
    run_with_client(
        parallel_backend,
        npartitions,
        bosonrecipes.ABDMC,
        scf_checkfile,
        abvmc_checkfile,
        **abdmc_params,
    )
    abdmc_end_time = time.time()
    abdmc_duration = abdmc_end_time - abdmc_start_time
    print(
        f"=== ABDMC STATISTICS calculation completed in {abdmc_duration:.2f} seconds "
        f"({abdmc_duration/60:.2f} minutes) ==="
    )

    end_time = time.time()
    total_duration = end_time - start_time
    print(
        f"=== Total execution time: {total_duration:.2f} seconds "
        f"({total_duration/60:.2f} minutes) ==="
    )
    print(f"=== Calculation completed at {time.strftime('%Y-%m-%d %H:%M:%S')} ===")
